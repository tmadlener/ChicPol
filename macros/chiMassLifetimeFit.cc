...
void chiMassLifetimeFit(const std::string &infilename, int rapBin, int ptBin, int nState, bool runChiMassFitOnly, bool MC, bool MCclosure=false){
  ...
  if(ptBin > 2){
    ws->var("BK_p1")->setVal(-0.255);
    ws->var("CBalpha1")->setVal(0.53);
    ws->var("CBalpha2")->setVal(0.49);
    ws->var("CBsigma1")->setVal(0.0056);
  }
  ...
  if(ptBin>3){
    ...
    // ws->var("CBmass1")->setConstant(kTRUE); // Ilse 15Feb2016
  }
