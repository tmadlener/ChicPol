...
double plotMassRap(RooWorkspace *ws, int rapBin, int ptBin, int nState, bool SpeedPlotting, int nSpeedPlotting, bool zoom){
	...
	const int colorRap[nRapSteps]={632, 600, 418, 616};
	...
	for(int iRapStep=0;iRapStep<nRapSteps;iRapStep++){
	    ...
	    cout<<"color "<<colorRap[iRapStep]<<endl;
		...
		dataRapStep[iRapStep]->plotOn(massFrame,MarkerSize(0.8), MarkerColor(colorRap[iRapStep]));
	    ...
	    FullmassPdf.plotOn(massFrame,
				LineWidth(2), LineColor(colorRap[iRapStep]),
				Normalization(nEntries,2)
	    );


		legendPhantom[iRapStep] = dataRapStep[iRapStep]->createHistogram(Form("legendRap%d",iRapStep),*JpsiMass,Binning(50)) ; legendPhantom[iRapStep]->SetLineColor(colorRap[iRapStep]) ; legendPhantom[iRapStep]->SetLineStyle(kSolid) ; legendPhantom[iRapStep]->SetLineWidth(2) ;
