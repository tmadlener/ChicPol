...
TH1F *events_NPSR[onia::kNbRapForPTBins][onia::kNbPTMaxBins];
...
int evtPinPRSR[onia::kNbRapForPTBins][onia::kNbPTMaxBins],
		...
		evtPinNPSR[onia::kNbRapForPTBins][onia::kNbPTMaxBins],
		evtNPinNPSR[onia::kNbRapForPTBins][onia::kNbPTMaxBins],
		evtBGinNPSR[onia::kNbRapForPTBins][onia::kNbPTMaxBins];
	...
	if(SaveTables){
		for(int iRap = 0; iRap < onia::kNbRapForPTBins; iRap++){
			for(int iPT = 0; iPT < onia::kNbPTBins[iRap+1]; iPT++){
				...
				cout<<"evtPinNPSR  ["<<iRap<<"]["<<iPT<<"]: "<<evtPinNPSR[iRap][iPT]<<endl;
				cout<<"evtNPinNPSR ["<<iRap<<"]["<<iPT<<"]: "<<evtNPinNPSR[iRap][iPT]<<endl;
				cout<<"evtBGinNPSR ["<<iRap<<"]["<<iPT<<"]: "<<evtBGinNPSR[iRap][iPT]<<endl;
...
void LoadHistos(Int_t iRapBin, Int_t iPTBin, Int_t nState){
	...
	sprintf(name, "events_nonpromptSR");
	events_NPSR[iRapBin][iPTBin] = (TH1F*) fIn->Get(name);
	cout<<"rap"<<iRapBin+1<<" pt"<<iPTBin+1<<" non-Prompt signal in NPSR: "
		<<events_NPSR[iRapBin][iPTBin]->GetBinContent(2)<<endl;
  ...
  evtPinNPSR [iRapBin][iPTBin] = (int)events_NPSR[iRapBin][iPTBin]->GetBinContent(1);
  evtNPinNPSR[iRapBin][iPTBin] = (int)events_NPSR[iRapBin][iPTBin]->GetBinContent(2);
  evtBGinNPSR[iRapBin][iPTBin] = (int)events_NPSR[iRapBin][iPTBin]->GetBinContent(3);
