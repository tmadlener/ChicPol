...
void bkgHistos_chi(const std::string infilename, int rapBin, int ptBin, bool folding, bool MC, bool PolLSB, bool PolRSB, bool PolNP, int FracLSB, bool normApproach, bool subtractNP){
    ...
    TLorentzVector *lepP = 0, *lepN = 0, *jpsi = 0, *chic = 0, *chic_rf = 0;
    ...
    int nBins = 100;
    TH1D* pT_L   = new TH1D( "pTLSB", "pTLSB", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_R   = new TH1D( "pTRSB", "pTRSB", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_highct_L   = new TH1D( "pT_highct_LSB", "pT_highct_LSB", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_highct_R   = new TH1D( "pT_highcta_RSB", "pT_highct_RSB", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_NP1   = new TH1D( "pTNP", "pTNP", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_NP2   = new TH1D( "pTNP", "pTNP", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_PSR1   = new TH1D( "pTPSR", "pTPSR", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    TH1D* pT_PSR2   = new TH1D( "pTPSR", "pTPSR", nBins, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin]);
    ...
    double fracLSBpsi = ((RooRealVar*)ws->var("var_fLSBpsi"))->getValV();
    if(FracLSB!=-1){
        fracLSB1 = double(FracLSB)/100.;
        fracLSB2 = double(FracLSB)/100.;
        fracLSBpsi = double(FracLSB)/100.;
    }
    std::cout << "fLSB = " << fracLSBpsi << " for Jpsi, " << fracLSB1 << " for chic1 and " << fracLSB2 << " for chic2" << std::endl;
    ...
    double fCombBgToTotBg = ((RooRealVar*)ws->var("var_fractionCombBGofTotalBackground"))->getValV(); // fraction of combinatorial mumu + gamma events to total background
    double fJpsiBgToTotBg = ((RooRealVar*)ws->var("var_fractionJpsiBGofTotalBackground"))->getValV(); // fraction of combinatorial Jpsi + gamma events to total background
    //std::cout << "fraction of mumu + gamma and Jpsi + gamma events relative to total background (should be 1): " << fCombBgToTotBg+fJpsiBgToTotBg << std::endl;
    double fCombBGInPRSR1 = fCombBgToTotBg*fBGsig1; // fraction of combinatorial mumu + gamma events in PRSR1
    double fCombBGInPRSR2 = fCombBgToTotBg*fBGsig2; // fraction of combinatorial mumu + gamma events in PRSR2
    double fJpsiBGInPRSR1 = fJpsiBgToTotBg*fBGsig1; // fraction of combinatorial Jpsi + gamma events in PRSR1
    double fJpsiBGInPRSR2 = fJpsiBgToTotBg*fBGsig2; // fraction of combinatorial Jpsi + gamma events in PRSR2
    //std::cout << "combinatorial mumu + gamma background fraction: " << fCombBGInPRSR1 << " ; (1-fJpsi) " << (1-fJpsiBgToTotBg)*fBGsig1 << std::endl;
    ...
    RooAbsPdf *sigMassJpsi = (RooAbsPdf*)ws->pdf("sigMassShape_jpsi");
    ...
    TF1* funcSigJpsi = (TF1*)sigMassJpsi->asTF(*mjpsi, RooArgList(*CBmassJpsi, *CBsigmaJpsi, *CBalphaJpsi, *CBnJpsi), *mjpsi);
    ...
    double jpsiMassPeak = ((RooRealVar*)ws->var("JpsiMass"))->getVal();
    double p0 = ((RooRealVar*)ws->var("CBsigma_p0_jpsi"))->getVal();
    double p1 = ((RooRealVar*)ws->var("CBsigma_p1_jpsi"))->getVal();
    double p2 = ((RooRealVar*)ws->var("CBsigma_p2_jpsi"))->getVal();
    double CBmass_p0 = ((RooRealVar*)ws->var("CBmass_p0_jpsi"))->getVal();
    double jpsiMassSigma = rapSigma(p0, p1, p2, onia::rapForPTRange[onia::kNbRapForPTBins]);
    std::cout << "sigma = " << jpsiMassSigma << std::endl;
    ...
    std::cout << "Jpsi mass signal region: " << jpsiMassSigMin << " - " << jpsiMassSigMax << std::endl;
    ...
    // prompt mass sidebands using background function (mumu + gamma events)
    TH3D* hBG1_pTRapMass_L = new TH3D("hBG_pTRapMass_L", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                      7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                      2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                      7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    ...
    // prompt mass sidebands using signal function (Jpsi + gamma events)
    TH3D* hBGJpsi1_pTRapMass_L = new TH3D("hBG_Jpsi_pTRapMass_L", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                          7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                          2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                          7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi1_pTRapMass_L->Sumw2();

    TH3D* hBGJpsi2_pTRapMass_L = new TH3D("hBG_Jpsi_pTRapMass_L", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                          7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                          2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                          7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi2_pTRapMass_L->Sumw2();

    TH3D* hBGJpsi1_pTRapMass_R = new TH3D("hBG_Jpsi_pTRapMass_R", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                          7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                          2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                          7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi1_pTRapMass_R->Sumw2();

    TH3D* hBGJpsi2_pTRapMass_R = new TH3D("hBG_Jpsi_pTRapMass_R", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                          7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                          2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                          7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi2_pTRapMass_R->Sumw2();


    // non prompt mass sidebands using background function (mumu + gamma events)
    ...
    // non prompt mass sidebands using signal function (Jpsi + gamma events)
    TH3D* hBGJpsi1_pTRapMass_highct_L = new TH3D("hBG_Jpsi_pTRapMass_highct_L", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                                 7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                                 2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                                 7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi1_pTRapMass_highct_L->Sumw2();

    TH3D* hBGJpsi2_pTRapMass_highct_L = new TH3D("hBG_Jpsi_pTRapMass_highct_L", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                                 7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                                 2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                                 7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi2_pTRapMass_highct_L->Sumw2();

    TH3D* hBGJpsi1_pTRapMass_highct_R = new TH3D("hBG_Jpsi_pTRapMass_highct_R", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                                 7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                                 2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                                 7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi1_pTRapMass_highct_R->Sumw2();

    TH3D* hBGJpsi2_pTRapMass_highct_R = new TH3D("hBG_Jpsi_pTRapMass_highct_R", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                                 7, onia::pTRange[rapBin][ptBin-1], onia::pTRange[rapBin][ptBin],
                                                 2, onia::rapForPTRange[rapBin-1], onia::rapForPTRange[rapBin], //onia::kNbRapForPTBins
                                                 7, jpsiMassSigMin, jpsiMassSigMax); // signal mass window!
    hBGJpsi2_pTRapMass_highct_R->Sumw2();

    // --- non prompt signal region
    ...
    //------ 3D histograms for LSB and RSB polarization
    ...
    for(int i = 0; i < n; i++){
        ...
        // ------------------------- TLorentzVecotrs -------------------------
        if(jpsi->Pt() >= onia::pTRange[rapBin][ptBin-1] &&
           jpsi->Pt() < onia::pTRange[rapBin][ptBin] &&
           ...
           TMath::Abs(jpsi->Rapidity()) < onia::rapForPTRange[rapBin]){
            ...
            if(TMath::Abs(jpsi->M() - CBmass_p0) < onia::nSigMass * rapSigma(p0, p1, p2, jpsi->Rapidity()) ){
                ...
                if(PolLSB){
                    if(chic_rf->M() > massMinL && chic_rf->M() < massMaxL && jpsict > PRmin && jpsict < PRmax){
                        outtree1->Fill();
                        outtree2->Fill();
                        pT_PSR1->Fill(jpsi->Pt());
                        rap_PSR1->Fill(TMath::Abs(jpsi->Rapidity()));
                        pT_PSR2->Fill(jpsi->Pt());
                        rap_PSR2->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                } // PolLSB
                // right sideband
                else if(PolRSB){
                ...
                if(MC){
                    ...
                    if(chic_rf->M() > massMinSR1 && chic_rf->M() < massMaxSR1){
                        ...
                        pT_NP1->Fill(jpsi->Pt());
                        rap_NP1->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                    else if(chic_rf->M() > massMinSR2 && chic_rf->M() < massMaxSR2){
                        ...
                        pT_NP2->Fill(jpsi->Pt());
                        rap_NP2->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                    ...
                    if(i%3==0){
                        hBG1_pTRapMass_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBG2_pTRapMass_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBG1_pTRapMass_highct_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBG2_pTRapMass_highct_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi1_pTRapMass_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi1_pTRapMass_highct_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_highct_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                    }
                    else if(i%5==0){
                        hBG1_pTRapMass_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBG2_pTRapMass_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBG1_pTRapMass_highct_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBG2_pTRapMass_highct_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi1_pTRapMass_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi1_pTRapMass_highct_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_highct_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), gRandom->Uniform(jpsiMassSigMin, jpsiMassSigMax));
                    }
                } // if (MC)
                    ...
                    if(chic_rf->M() > massMinL && chic_rf->M() < massMaxL && jpsict > PRmin && jpsict < PRmax){
                        ...
                        hBGJpsi1_pTRapMass_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        pT_L->Fill(jpsi->Pt());
                        rap_L->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                        ...
                        hBGJpsi1_pTRapMass_highct_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_highct_L->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        pT_highct_L->Fill(jpsi->Pt());
                        rap_highct_L->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                    else if(chic_rf->M() > massMinSR1 && chic_rf->M() < massMaxSR1 && jpsict > PRmin && jpsict < PRmax){
                        index = 2;
                        hSR1_pTRapMass->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                    }
                    else if(chic_rf->M() > massMinSR1 && chic_rf->M() < massMaxSR1 && jpsict > NPmin && jpsict < NPmax){
                        index = 3;
                        hNP1_pTRapMass->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        pT_NP1->Fill(jpsi->Pt());
                        rap_NP1->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                    else if(chic_rf->M() > massMinSR2 && chic_rf->M() < massMaxSR2 && jpsict > PRmin && jpsict < PRmax){
                        ...
                        index = 5;
                        hNP2_pTRapMass->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        pT_NP2->Fill(jpsi->Pt());
                        rap_NP2->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                        ...
                        hBGJpsi1_pTRapMass_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        pT_R->Fill(jpsi->Pt());
                        rap_R->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                        ...
                        hBGJpsi1_pTRapMass_highct_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        hBGJpsi2_pTRapMass_highct_R->Fill(jpsi->Pt(), TMath::Abs(jpsi->Rapidity()), funcSigJpsi->GetRandom(jpsiMassSigMin, jpsiMassSigMax));
                        pT_highct_R->Fill(jpsi->Pt());
                        rap_highct_R->Fill(TMath::Abs(jpsi->Rapidity()));
                    }
                    else continue;
            ...
            } // if(onia...) restrict to SR jpsi mass region
        } // if(onia...)
    } // i
    ...
    for(int i = 0; i < n; i++){
        ...
        if(jpsi->Pt() >= onia::pTRange[rapBin][ptBin-1] &&
           ...
           TMath::Abs(jpsi->Rapidity()) < onia::rapForPTRange[rapBin]){
            ...
            } // if(onia...) restrict jpsi mass to SR
        } // if(onia...)
    } // i
//------loop finished
...
// write background histos to file
    ...
    hBGJpsi1_pTRapMass_L->Write();
    hBGJpsi1_pTRapMass_R->Write();
    hSR1_pTRapMass->Write();
    hBG1_pTRapMass_highct_L->Write();
    hBG1_pTRapMass_highct_R->Write();
    hBGJpsi1_pTRapMass_highct_L->Write();
    hBGJpsi1_pTRapMass_highct_R->Write();
    hNP1_pTRapMass->Write();
    output2->cd();
    hBG2_pTRapMass_L->Write();
    hBG2_pTRapMass_R->Write();
    hBGJpsi2_pTRapMass_L->Write();
    hBGJpsi2_pTRapMass_R->Write();
    hSR2_pTRapMass->Write();
    hBG2_pTRapMass_highct_L->Write();
    hBG2_pTRapMass_highct_R->Write();
    hBGJpsi2_pTRapMass_highct_L->Write();
    hBGJpsi2_pTRapMass_highct_R->Write();
...
// mumu + gamma
    ...
    std::string namepTrapMasslowctMuMu = "combMuMu_background_pTrapMass";
    TH3D* hBG1_pTRapMass_lowct = (TH3D*) hBG1_pTRapMass_L->Clone(namepTrapMasslowctMuMu.c_str());
    hBG1_pTRapMass_lowct->Add(hBG1_pTRapMass_R);
    output1->cd();
    hBG1_pTRapMass_lowct->Write();
// Jpsi + gamma
    hBGJpsi1_pTRapMass_L->Scale(fracLSB1/(1.*hBGJpsi1_pTRapMass_L->Integral()));
    hBGJpsi1_pTRapMass_R->Scale((1.-fracLSB1)/(1.*hBGJpsi1_pTRapMass_R->Integral()));
    std::string namepTrapMasslowctJpsi = "combJpsi_background_pTrapMass";
    TH3D* hBGJpsi1_pTRapMass_lowct = (TH3D*) hBGJpsi1_pTRapMass_L->Clone(namepTrapMasslowctJpsi.c_str());
    hBGJpsi1_pTRapMass_lowct->Add(hBGJpsi1_pTRapMass_R);
    output1->cd();
    hBGJpsi1_pTRapMass_lowct->Write();
// combine combinatorial background
    hBG1_pTRapMass_lowct->Scale(fCombBgToTotBg/(1.*hBG1_pTRapMass_lowct->Integral()));
    hBGJpsi1_pTRapMass_lowct->Scale((1.-fCombBgToTotBg)/(1.*hBGJpsi1_pTRapMass_lowct->Integral()));
    hBG1_pTRapMass_lowct->Add(hBGJpsi1_pTRapMass_lowct);
    std::string namepTrapMasslowct = "comb_background_pTrapMass";
    hBG1_pTRapMass_lowct->SetName(namepTrapMasslowct.c_str());
    output1->cd();
    hBG1_pTRapMass_lowct->Write();
// chic2
// mumu + gamma
    ...
    TH3D* hBG2_pTRapMass_lowct = (TH3D*) hBG2_pTRapMass_L->Clone(namepTrapMasslowctMuMu.c_str());
    hBG2_pTRapMass_lowct->Add(hBG2_pTRapMass_R);
    output2->cd();
    hBG2_pTRapMass_lowct->Write();
// Jpsi + gamma
    hBGJpsi2_pTRapMass_L->Scale(fracLSB2/(1.*hBGJpsi2_pTRapMass_L->Integral()));
    hBGJpsi2_pTRapMass_R->Scale((1.-fracLSB2)/(1.*hBGJpsi2_pTRapMass_R->Integral()));
    TH3D* hBGJpsi2_pTRapMass_lowct = (TH3D*) hBGJpsi2_pTRapMass_L->Clone(namepTrapMasslowctJpsi.c_str());
    hBGJpsi2_pTRapMass_lowct->Add(hBGJpsi2_pTRapMass_R);
    output2->cd();
    hBGJpsi2_pTRapMass_lowct->Write();
// combine combinatorial background
    hBG2_pTRapMass_lowct->Scale(fCombBgToTotBg/(1.*hBG2_pTRapMass_lowct->Integral()));
    hBGJpsi2_pTRapMass_lowct->Scale((1.-fCombBgToTotBg)/(1.*hBGJpsi2_pTRapMass_lowct->Integral()));
    hBG2_pTRapMass_lowct->Add(hBGJpsi2_pTRapMass_lowct);
    hBG2_pTRapMass_lowct->SetName(namepTrapMasslowct.c_str());
    output2->cd();
    hBG2_pTRapMass_lowct->Write();
...
// mumu + gamma
    ...
    std::string namepTrapMasshighctMuMu = "combMuMu_background_highct_pTrapMass";
    TH3D* hBG1_pTRapMass_highct = (TH3D*) hBG1_pTRapMass_highct_L->Clone(namepTrapMasshighctMuMu.c_str());
    hBG1_pTRapMass_highct->Add(hBG1_pTRapMass_highct_R);
    output1->cd();
    hBG1_pTRapMass_highct->Write();
// Jpsi + gamma
    hBGJpsi1_pTRapMass_highct_L->Scale(fracLSB1/(1.*hBGJpsi1_pTRapMass_highct_L->Integral()));
    hBGJpsi1_pTRapMass_highct_R->Scale((1.-fracLSB1)/(1.*hBGJpsi1_pTRapMass_highct_R->Integral()));
    std::string namepTrapMasshighctJpsi = "combJpsi_background_highct_pTrapMass";
    TH3D* hBGJpsi1_pTRapMass_highct = (TH3D*) hBGJpsi1_pTRapMass_highct_L->Clone(namepTrapMasshighctJpsi.c_str());
    hBGJpsi1_pTRapMass_highct->Add(hBGJpsi1_pTRapMass_highct_R);
    output1->cd();
    hBGJpsi1_pTRapMass_highct->Write();
// combine combinatorial background
    hBG1_pTRapMass_highct->Scale(fCombBgToTotBg/(1.*hBG1_pTRapMass_highct->Integral()));
    hBGJpsi1_pTRapMass_highct->Scale((1.-fCombBgToTotBg)/(1.*hBGJpsi1_pTRapMass_highct->Integral()));
    hBG1_pTRapMass_highct->Add(hBGJpsi1_pTRapMass_highct);
    std::string namepTrapMasshighct = "comb_background_highct_pTrapMass";
    hBG1_pTRapMass_highct->SetName(namepTrapMasshighct.c_str());
    output1->cd();
    hBG1_pTRapMass_highct->Write();
// chic2
// mumu + gamma
    ...
    TH3D* hBG2_pTRapMass_highct = (TH3D*) hBG2_pTRapMass_highct_L->Clone(namepTrapMasshighctMuMu.c_str());
    hBG2_pTRapMass_highct->Add(hBG2_pTRapMass_highct_R);
    output2->cd();
    hBG2_pTRapMass_highct->Write();
// Jpsi + gamma
    hBGJpsi2_pTRapMass_highct_L->Scale(fracLSB2/(1.*hBGJpsi2_pTRapMass_highct_L->Integral()));
    hBGJpsi2_pTRapMass_highct_R->Scale((1.-fracLSB2)/(1.*hBGJpsi2_pTRapMass_highct_R->Integral()));
    TH3D* hBGJpsi2_pTRapMass_highct = (TH3D*) hBGJpsi2_pTRapMass_highct_L->Clone(namepTrapMasshighctJpsi.c_str());
    hBGJpsi2_pTRapMass_highct->Add(hBGJpsi2_pTRapMass_highct_R);
    output2->cd();
    hBGJpsi2_pTRapMass_highct->Write();
// combine combinatorial background
    hBG2_pTRapMass_highct->Scale(fCombBgToTotBg/(1.*hBG2_pTRapMass_highct->Integral()));
    hBGJpsi2_pTRapMass_highct->Scale((1.-fCombBgToTotBg)/(1.*hBGJpsi2_pTRapMass_highct->Integral()));
    hBG2_pTRapMass_highct->Add(hBGJpsi2_pTRapMass_highct);
    hBG2_pTRapMass_highct->SetName(namepTrapMasshighct.c_str());
    output2->cd();
    hBG2_pTRapMass_highct->Write();
...
// chic1
    ...
    TH3D *hBG1_pTRapMass_highct_temp = (TH3D*) hBG1_pTRapMass_highct->Clone();
    hBG1_pTRapMass_highct_temp->Scale(fBGinNP1/(1.*hBG1_pTRapMass_highct_temp->Integral()));
    std::string namepTrapMassNPS = "NPS_highct_pTrapMass";
    TH3D* hNPS1_pTRapMass = (TH3D*) hNP1_pTRapMass->Clone(namepTrapMassNPS.c_str());
    hNPS1_pTRapMass = subtract3D(hNPS1_pTRapMass, hBG1_pTRapMass_highct_temp);
    output1->cd();
    hNPS1_pTRapMass->Write();
// chic2
    hNP2_pTRapMass->Scale(1./(1.*hNP2_pTRapMass->Integral()));
    TH3D *hBG2_pTRapMass_highct_temp = (TH3D*) hBG2_pTRapMass_highct->Clone();
    hBG2_pTRapMass_highct_temp->Scale(fBGinNP2/(1.*hBG2_pTRapMass_highct_temp->Integral()));
    TH3D* hNPS2_pTRapMass = (TH3D*) hNP2_pTRapMass->Clone(namepTrapMassNPS.c_str());
    hNPS2_pTRapMass = subtract3D(hNPS2_pTRapMass, hBG2_pTRapMass_highct_temp);
    output2->cd();
