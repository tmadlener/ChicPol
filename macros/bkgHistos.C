...
//---------------------------------------------------------------------------------------------------------------
void bkgHistos(const std::string infilename, int rapBin, int ptBin, int nState, bool folding, bool MC, bool doCtauUncer, bool PolLSB, bool PolRSB, bool PolNP, int ctauScen, int FracLSB, bool forceBinning, bool normApproach, bool scaleFracBg, const std::string& polDataPath){
  ...
  // calculate fractions in the non-prompt signal region
  ct->setRange("NPR", ctCutMinNP, ctCutMaxNP);
  // prompt fraction in signal region
  RooAbsReal* i_fPRinNPSR = PRpdf->createIntegral(RooArgSet(*ct), NormSet(RooArgSet(*ct)), Range("NPR"));
  double fPinNPSR = i_fPRinNPSR->getVal();
  // non prompt fraction in signal region
  RooAbsReal* i_fNPRinNPSR = NPpdf->createIntegral(RooArgSet(*ct), NormSet(RooArgSet(*ct)), Range("NPR"));
  double fNPinNPSR = i_fNPRinNPSR->getVal();
  // background fraction in signal region
  RooAbsReal* i_fBkginNPSR = BGpdf->createIntegral(RooArgSet(*ct), NormSet(RooArgSet(*ct)), Range("NPR"));
  double fBGinNPSR = i_fBkginNPSR->getVal();
  ...
  std::cout << "-------------------------------------------------------------\n" <<
    ...
    "-------------------------------------------------------------\n" <<
    "fraction of prompt events in NPSR: " << fPinNPSR << " \n" <<
    "fraction of non prompt events in NPSR: " << fNPinNPSR << " \n" <<
    "fraction of background events in NPSR: " << fBGinNPSR << " \n" <<
    "-------------------------------------------------------------\n" <<
  ...
  if(scaleFracBg){
    //fTBGsig = 0.001;
    char Filename[200];
    sprintf(Filename,"%s/results_Psi%dS_rap%d_pT%d.root",polDataPath.c_str(),nState-3,rapBin,ptBin);
  ...
  std::stringstream evtNPSRname;
  evtNPSRname << ";;events in " << onia::nSigMass << "  sigma window and high ctau region";
  TH1D* hEvtNPSR = new TH1D("events_nonpromptSR", evtNPSRname.str().c_str(), 3, 0., 3.);
  // fill histogram only for data
  if(!MC){
    hEvtNPSR->SetBinContent(1, fPinNPSR*nP);
    hEvtNPSR->SetBinContent(2, fNPinNPSR*nNP);
    hEvtNPSR->SetBinContent(3, fBGinNPSR*nBG);
    hEvtNPSR->Write();
  }
