...
//			  eff = graph2D->Interpolate(hEvalEff1D->GetYaxis()->GetBinCenter(pTBin+1),hEvalEff1D->GetXaxis()->GetBinCenter(etaBin+1));
    ...
    std::cout << "checked fInEff" << std::endl;


    // check different efficiency files
    TEfficiency* TEff;
    TH2D* hEvalEff;
    std::cout << "created dummy histos" << std::endl;
    ...
    std::cout << "explored different nEff options" << std::endl;
    
    if(nEff!=105 && nEff!=106 && nEff!=1){
        if(nEff < 100000)
            hEvalEff = (TH2D*)hEvalEff1D->Clone("hEvalEff");        
    }
    else if(nEff > 10000 && nEff < 100000){
        hEvalEff = (TH2D*)hEvalEff2D->Clone("hEvalEff");
    }
    ...
    std::cout << "Amap file opened" << std::endl;
    ...
    if(useAmapApproach){
        ...
        else if(nDenominatorAmap==105 || nDenominatorAmap==106){
            sprintf(EffType,"totEff_MCTRUTH_pT_eta");
  ...
  // boundaries of the kinematic cell taken from the background histograms:
    ...
    cout << "pTdilepton_min " << pTdilepton_min << endl;
    cout << "pTdilepton_max " << pTdilepton_max << endl;
    ...
    for(int i_MCevent = 1; i_MCevent <= n_MCevents_ACC; i_MCevent++){
        ...
        if (i_MCevent%n_step == 0) {
            cout << n_step_*20 <<" % "<<endl;
            cout << "pt = " << pT << ", rap = " << rap << ", mass = " << mass << endl;
            n_step_++;
        }
...
//    cout<<"costh_PX = "<<costh_PX<<", phi_PX = "<<phi_PX<<endl;
        ...
        // efficiencies
        //std::cout << "get single muon efficiency: " << lepP_pT << ", " << lepP_eta << "; " << lepN_pT << ", " << lepN_eta << std::endl;
        double epsilon = 0;
        if(accept){
            if(nEff > 100000){
                int etaBin = -1.;
        ...
        else if(nDileptonEff>210 && nDileptonEff<221) {costh_DILEff=costh_HX; phi_DILEff=phi_HX;}
        else if(nDileptonEff>220 && nDileptonEff<231) {costh_DILEff=costh_PX; phi_DILEff=phi_PX;}
        ...
        else if(nRhoFactor>310 && nRhoFactor<321) {costh_RhoFactor=costh_HX; phi_RhoFactor=phi_HX;}
        else if(nRhoFactor>320 && nRhoFactor<331) {costh_RhoFactor=costh_PX; phi_RhoFactor=phi_PX;}
...
*/
        ...
        if ( isEventAccepted ) {
            ...
            else if(NewAccCalc) acceptance->Fill( pT, TMath::Abs(rap), mass ,epsilon );
        }
  ...
  // failsafe for background (should not exceed signal+background)
  for(int iX=0;iX<background_costhphiPX->GetNbinsX()+1;iX++){
      for(int iY=0;iY<background_costhphiPX->GetNbinsY()+1;iY++){
          double BG_content=background_costhphiPX->GetBinContent(iX,iY);
          double Total_content=total_costhphiPX->GetBinContent(iX,iY);
          if(BG_content>Total_content){
              background_costhphiPX->SetBinContent(iX,iY,Total_content);
              cout<<"Adapted background model histogram content (bkg = " << BG_content << ", tot = " << Total_content << ")at bin X="<<iX<<", Y="<<iY<<endl;
          }
      }
  }
  
  background_costhPX = background_costhphiPX->ProjectionX( "background_costhPX" );
  background_phiPX   = background_costhphiPX->ProjectionY( "background_phiPX" );  
  background_costhPX->Scale( f_background*n_events / background_costhPX->Integral() );
  background_phiPX->Scale( f_background*n_events / background_phiPX->Integral() );
  ...
  for ( int i_event = 1; i_event <= n_events; i_event++ ) {
    ...
    // total likelihood
      ...
      if (    likelihood_BG > likelihood_SpB * f_background
              || likelihood_BG > likelihood_SpB * gRandom->Uniform( 1. )  ) { //Ilse 2Feb 2016 gRandom->Uniform( f_background )
...
*/
  ...
  double lth_CS_1 = 0, lph_CS_1 = 0, ltp_CS_1 = 0, lthstar_CS_1 = 0, lphstar_CS_1 = 0, ltilde_CS_1 = 0;
  double lth_HX_1 = 0, lph_HX_1 = 0, ltp_HX_1 = 0, lthstar_HX_1 = 0, lphstar_HX_1 = 0, ltilde_HX_1 = 0;
  double lth_PX_1 = 0, lph_PX_1 = 0, ltp_PX_1 = 0, lthstar_PX_1 = 0, lphstar_PX_1 = 0, ltilde_PX_1 = 0;
  int positivity_CS_1 = 1, positivity_HX_1 = 1, positivity_PX_1 = 1;
...
//      if (i_sampledPoint%100 == 0) {cout << "point " << i_sampledPoint << " of " << n_sampledPoints << endl; }
      
      //int step = 0; //value to keep track of whether to go one step back to previously sampled point
     ...
     // new: previous value is added to file in case value is rejected
     ...
     if(  loglikelihood_CS_difference > 0.  ||  log( gRandom->Uniform(1.) ) < loglikelihood_CS_difference  ) {
         ...
         if ( i_sampledPoint > n_burnIn ) {  // reject first n_burnIn extractions in the chain
             ...
             lph_CS_1 = lph_CS;
             ltp_CS_1 = ltp_CS;
             lthstar_CS_1 = lthstar_CS;
             lphstar_CS_1 = lphstar_CS;
             ltilde_CS_1 = ltilde_CS;
             positivity_CS_1 = positivity_CS;
             //cout << "--CS--" << "\n" 
             //     << "lth = " << lth_CS << "\n"
             //     << "lph = " << lph_CS << "\n"
             //     << "ltp = " << ltp_CS << endl;
         }
         else if ( i_sampledPoint > n_burnIn/2 ) { 
             lth_CS_1 = lth_CS;
             lph_CS_1 = lph_CS;
             ltp_CS_1 = ltp_CS;
             lthstar_CS_1 = lthstar_CS;
         ...
         if(i_sampledPoint > n_burnIn){ 
             //in case of rejection write lambda_{i} to file
             lth_CS = lth_CS_1;
             lph_CS = lph_CS_1;
             ltp_CS = ltp_CS_1;
             lthstar_CS = lthstar_CS_1;
             lphstar_CS = lphstar_CS_1;
             ltilde_CS = ltilde_CS_1;
             positivity_CS = positivity_CS_1;
             //cout << "lth1_CS = " << lth_CS_1 << "\n"
             //     << "lph1_CS = " << lph_CS_1 << "\n"
             //     << "ltp1_CS = " << ltp_CS_1 << "\n"
             //     << "-------------------" << endl;
             results_CS->Fill();
     ...
     if ( i_sampledPoint == n_burnIn +1 ) { 
         proposalWidth_lth_CS = TMath::Max( proposalWidthTimesRMS * test_lth_CS->GetRMS(), 0.001 );
     ...
     if(  loglikelihood_HX_difference > 0.  ||  log( gRandom->Uniform(1.) ) < loglikelihood_HX_difference  ) {
         ...
         if ( i_sampledPoint > n_burnIn ) {
             ...
             lph_HX_1 = lph_HX;
             ltp_HX_1 = ltp_HX;
             lthstar_HX_1 = lthstar_HX;
             lphstar_HX_1 = lphstar_HX;
             ltilde_HX_1 = ltilde_HX;
             positivity_HX_1 = positivity_HX;
             //cout << "--HX--" << "\n" 
             //     << "lth = " << lth_HX << "\n"
             //     << "lph = " << lph_HX << "\n"
             //     << "ltp = " << ltp_HX << endl;
         ...
         if(i_sampledPoint > n_burnIn){
             ...
             lph_HX = lph_HX_1;
             ltp_HX = ltp_HX_1;
             lthstar_HX = lthstar_HX_1;
             lphstar_HX = lphstar_HX_1;
             ltilde_HX = ltilde_HX_1;
             positivity_HX = positivity_HX_1;
             //cout << "lth1_HX = " << lth_HX_1 << "\n"
             //     << "lph1_HX = " << lph_HX_1 << "\n"
             //     << "ltp1_HX = " << ltp_HX_1 << "\n"
             //     << "----------------------" << endl;
             results_HX->Fill();
     ...
     if ( i_sampledPoint == n_burnIn +1 ) { 
         proposalWidth_lth_HX = TMath::Max( proposalWidthTimesRMS * test_lth_HX->GetRMS(), 0.001 );
     ...
     if(  loglikelihood_PX_difference > 0.  ||  log( gRandom->Uniform(1.) ) < loglikelihood_PX_difference  ) {
         ...
         if ( i_sampledPoint > n_burnIn ) {
             ...
             lph_PX_1 = lph_PX;
             ltp_PX_1 = ltp_PX;
             lthstar_PX_1 = lthstar_PX;
             lphstar_PX_1 = lphstar_PX;
             ltilde_PX_1 = ltilde_PX;
             positivity_PX_1 = positivity_PX;
             //cout << "--PX--" << "\n" 
             //     << "lth = " << lth_PX << "\n"
             //     << "lph = " << lph_PX << "\n"
             //     << "ltp = " << ltp_PX << endl;
         ...
         if(i_sampledPoint > n_burnIn) {
             ...
             lph_PX = lph_PX_1;
             ltp_PX = ltp_PX_1;
             lthstar_PX = lthstar_PX_1;
             lphstar_PX = lphstar_PX_1;
             ltilde_PX = ltilde_PX_1;
             positivity_PX = positivity_PX_1;
             results_PX->Fill(); 
             rejectPointPX++;
             //cout << "lth_PX = " << lth_HX << "\n"
             //     << "lph_PX = " << lph_HX << "\n"
             ...
             //     << "----------------------" << endl;
             //step = 1;
             i_sampledPoint=i_sampledPoint-1;
         }
     }
     if ( i_sampledPoint == n_burnIn +1 ) { 
         proposalWidth_lth_PX = TMath::Max( proposalWidthTimesRMS * test_lth_PX->GetRMS(), 0.001 );
...
//  TH1D* h_lthstar_PX = new TH1D( "h_lthstar_PX", "", l_bins, results_PX->GetMinimum("lthstar"), results_PX->GetMaximum("lthstar") );
  ...
  TH1D* h_lth_CS = new TH1D( "h_lth_CS", "", l_bins, -1, 1 );
  TH1D* h_lph_CS = new TH1D( "h_lph_CS", "", l_bins, -1, 1 );
  TH1D* h_ltp_CS = new TH1D( "h_ltp_CS", "", l_bins, -1, 1 );
  TH1D* h_ltilde_CS = new TH1D( "h_ltilde_CS", "", l_bins, -1, 1);
  TH1D* h_lphstar_CS = new TH1D( "h_lphstar_CS", "", l_bins, -1, 1 );
  TH1D* h_lthstar_CS = new TH1D( "h_lthstar_CS", "", l_bins, -1, 1 );
  
  TH1D* h_lth_HX = new TH1D( "h_lth_HX", "", l_bins, -1, 1 );
  TH1D* h_lph_HX = new TH1D( "h_lph_HX", "", l_bins, -1, 1 );
  TH1D* h_ltp_HX = new TH1D( "h_ltp_HX", "", l_bins, -1, 1 );
  TH1D* h_ltilde_HX = new TH1D( "h_ltilde_HX", "", l_bins, -1, 1 );
  TH1D* h_lphstar_HX = new TH1D( "h_lphstar_HX", "", l_bins, -1, 1 );
  TH1D* h_lthstar_HX = new TH1D( "h_lthstar_HX", "", l_bins, -1, 1 );
  
  TH1D* h_lth_PX = new TH1D( "h_lth_PX", "", l_bins, -1, 1 );
  TH1D* h_lph_PX = new TH1D( "h_lph_PX", "", l_bins, -1, 1 );
  TH1D* h_ltp_PX = new TH1D( "h_ltp_PX", "", l_bins, -1, 1 );
  TH1D* h_ltilde_PX = new TH1D( "h_ltilde_PX", "", l_bins, -1, 1 );
  TH1D* h_lphstar_PX = new TH1D( "h_lphstar_PX", "", l_bins, -1, 1 );
  TH1D* h_lthstar_PX = new TH1D( "h_lthstar_PX", "", l_bins, -1, 1 );
