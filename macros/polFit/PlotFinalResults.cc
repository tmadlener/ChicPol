...
int main(int argc, char**  argv) {
  ...
  for( int i=0;i < argc; ++i ) {
    ...
    if(std::string(argv[i]).find("PlotForPaper=1") != std::string::npos) {PlotForPaper=true; cout<<"PlotForPaper"<<endl;}
  ...
  //sprintf(filename,"%s/macros/polFit/MattRes/TGraphResults_chic%d_MattSyst.root",basedir,nState-5);
  ...
  sprintf(StateDir,"%s/Chic%d",JobDir,nState-5);
    ...
    for(int rapBin = 1; rapBin < nRapBins+1; rapBin++){
      ...
      if(PlotForPaper){
        ...
        if(iLam==2||iLam==8||iLam==14||iLam==3||iLam==9||iLam==15){
          yMin=-0.34; //-0.399;
          yMax=0.34; //0.399;
          //if(iLam==3||iLam==9||iLam==15) yMin=-0.41;
        }
      ...
      if(PlotMatt&&PlotMattForICHEP){ //not yet changed 12.12.2012 Linlin
        ...
        char ICHEPtext[200];
        sprintf(ICHEPtext,"#psi(%dS)",nState-5);
      ...
      if(MultiPanelPlots&&iLam!=4&&iLam!=5&&iLam!=10&&iLam!=11&&iLam!=16&&iLam!=17){
        ...
        if(Psi_MPplots){
          ...
          if(iLam!=6&&iLam!=12&&iLam!=18){
            ...
            char texTexMP[200];
            if(rapBin==1) sprintf(texTexMP,"|#it{y}| < 0.6", nState-5, frameMPtex);
            if(rapBin==2) sprintf(texTexMP,"0.6 < |#it{y}| < 1.2", nState-5, frameMPtex);
            if(rapBin==3) sprintf(texTexMP,"1.2 < |#it{y}| < 1.5", nState-5, frameMPtex);
            ...
            if(rapBin==2&&iPanel==1){
              ...
              sprintf(textStateFrame,"#chi_{c%d}, %s", nState-5, frameMPtex);
        ...
        if(Psi_MPplots){
          ...
          if(iLam==6||iLam==12||iLam==18){
            ...
            char texTexMP[200];
            if(rapBin==1) sprintf(texTexMP,"#psi(%dS), |#it{y}| < 0.6", nState-5);
            if(rapBin==2) sprintf(texTexMP,"#psi(%dS), 0.6 < |#it{y}| < 1.2", nState-5);
            if(rapBin==3) sprintf(texTexMP,"#psi(%dS), 0.6 < |#it{y}| < 1.5", nState-5);
            if(nState==4){
            ...
            if((iLam==6||iLam==12||iLam==18)&&((nState<=4&&rapBin==2)||(nState==5&&rapBin==3))){
              if(mainframe==1) sprintf(filename,"%s/FinalResultsTildeCS_chic%d.pdf",FigDir,nState-5);
              if(mainframe==2) sprintf(filename,"%s/FinalResultsTildeHX_chic%d.pdf",FigDir,nState-5);
              if(mainframe==3) sprintf(filename,"%s/FinalResultsTildePX_chic%d.pdf",FigDir,nState-5);
        ...
        if(Psi_MPplots_New){
          ...
          if(iLam!=6&&iLam!=12&&iLam!=18&&rapBin==1){
            ...
            for(int iStateMP=1;iStateMP<totalState+1;iStateMP++){ //
              ...
              if(iStateMP==1){ PlotpTMin = 12.; PlotpTMax = 72.; }
              if(iStateMP==2){ PlotpTMin = 12.; PlotpTMax = 51.; }
              ...
              MPframedepLegend=new TLegend(0.65, 0.22, 0.85, 0.48);

              if(MPframe==1) MPframedepLegend=new TLegend(0.2, 0.22, 0.4, 0.48);
              ...
              TGaxis *axisMPY1 = new TGaxis(PlotpTMin-DeltaXminOVERALL,yMinMP,PlotpTMin-DeltaXminOVERALL,yMaxMP,
                                            yMinMP,yMaxMP,AxisDivisions,"-US");
              axisMPY1->SetTickSize(ticksize);
              if(iLam==2||iLam==8||iLam==14||iLam==3||iLam==9||iLam==15) axisMPY1->SetNdivisions(404);
              if(iLam==1||iLam==7||iLam==13)  axisMPY1->SetNdivisions(505);
              if(iPanel==nPanels) axisMPY1->SetTickSize(ticksize/(1-lowestBottomMargin));
              axisMPY1->Draw("same");

              TGaxis *axisMPY2 = new TGaxis(PlotpTMax,yMinMP,PlotpTMax,yMaxMP,yMinMP,yMaxMP,AxisDivisions,"+US");
              axisMPY2->SetTickSize(ticksize);
              if(iLam==2||iLam==8||iLam==14||iLam==3||iLam==9||iLam==15) axisMPY2->SetNdivisions(404);
              if(iLam==1||iLam==7||iLam==13)  axisMPY2->SetNdivisions(505);
              if(iPanel==nPanels) axisMPY2->SetTickSize(ticksize/(1-lowestBottomMargin));
              if(iStateMP==1) {
                axisMPY2->SetTickSize(ticksize*0.7);
                if(iPanel==nPanels) axisMPY2->SetTickSize(ticksize/(1-lowestBottomMargin)*0.7);
              }
              axisMPY2->Draw("same");
              ...
              TGaxis *axisMPX1 = new TGaxis(PlotpTMin-DeltaXminOVERALL,yMinMP,PlotpTMax,yMinMP,
                                            PlotpTMin-DeltaXminOVERALL+deltaTrickAxisMin,PlotpTMax+deltaTrickAxisMax,AxisDivisions,"+S");
              axisMPX1->SetTickSize(ticksize*2);
              if(iStateMP==2) axisMPX1->SetNdivisions(504);
              if(iPanel==nPanels) axisMPX1->SetLabelSize(LabelSize);
              if(iPanel<nPanels) axisMPX1->SetLabelSize(0);
              axisMPX1->SetLabelOffset(labelOffsetX);
              if(iPanel==nPanels) axisMPX1->SetTickSize(ticksize/(1-lowestBottomMargin));
              ...
              TGaxis *axisMPX2 = new TGaxis(PlotpTMin-DeltaXminOVERALL,yMaxMP,PlotpTMax,yMaxMP,
                                            PlotpTMin-DeltaXminOVERALL+deltaTrickAxisMin,PlotpTMax+deltaTrickAxisMax,AxisDivisions,"-US");
              axisMPX2->SetTickSize(ticksize*2);
              if(iStateMP==2) axisMPX2->SetNdivisions(504);
              if(iPanel==nPanels) axisMPX2->SetTickSize(ticksize/(1-lowestBottomMargin));
              axisMPX2->Draw("same");
              ...
              // draw X axis title using latex
              if(iStateMP==2 && iPanel==nPanels){
                char axistitleMPX[200];
                sprintf(axistitleMPX, "#it{p}_{T} [GeV]");
                TLatex *MPXtitletext = new TLatex(5., -0.45, axistitleMPX );
                MPXtitletext->SetTextSize(YaxistitleLatexSize*0.8);
                MPXtitletext->SetTextColor(kBlack);
                MPXtitletext->Draw( "same" );
              }
              if(iStateMP==1 && iPanel==nPanels){
                char axistitleMPX[200];
                sprintf(axistitleMPX, "#it{p}_{T} [GeV]");

                TLatex *MPXtitletext = new TLatex(65.1, -0.45, axistitleMPX );
                MPXtitletext->SetTextSize(YaxistitleLatexSize*0.8);
                MPXtitletext->SetTextColor(kBlack);
                MPXtitletext->Draw( "same" );
              }
              ...
              if(iStateMP==2) xRapText_MPnew = 44 - 3;
              ...
              if(iStateMP==1&&iPanel==1){
                ...
                sprintf(text,"CMS                      %s", frameMPtex);
                if(DrawPreliminary) sprintf(text,"CMS Preliminary          %s", frameMPtex);
                double MPlatexX_buffer = 16;
                TLatex *CentralsText1MP = new TLatex(MPlatexX_buffer,MPlatexYmax,text);
                CentralsText1MP->SetTextSize(CentralsFontSizeMP*1.1);
                CentralsText1MP->Draw( "same" );
              }
              ...
              if(iStateMP==1&&iPanel==2){
                ...
                double MPlatexX_buffer = 16;
                TLatex *CentralsText2MP = new TLatex(MPlatexX_buffer,MPlatexYmax*0.9,text); //0.9
                CentralsText2MP->SetTextSize(CentralsFontSizeMP*1.1); //0.9
                CentralsText2MP->Draw( "same" );
              }
              ...
              if(iStateMP==2&&iPanel==1){
                ...
                if(MPframe==3) sprintf(frameMPtex,"PX frame");
                char textStateFrame[200];
                sprintf(textStateFrame,"#psi(%dS), %s", nState-5, frameMPtex);
                if(nState==4) sprintf(textStateFrame,"J/#psi, %s", frameMPtex);
                TLatex *TexStateFrame = new TLatex(MPlatexX,MPlatexYmax,textStateFrame);
                TexStateFrame->SetTextSize(CentralsFontSizeMP);
                //TexStateFrame->Draw( "same" );
        ...
        if(Psi_MPplots_New){
          ...
          if( (iLam==6||iLam==12||iLam==18 ) && rapBin<3 ){
            ...
            for(int iStateMP=1;iStateMP<totalState+1;iStateMP++){ //
              ...
              cout<<"MultiPanel hist"<<endl;
              TH1F *MPhist = new TH1F;
              if(rapBin==1) MPhist = MPcanvasTilde_rap1->DrawFrame(PlotpTMin-DeltaXminOVERALL,yMinMP,PlotpTMax,yMaxMP);
              if(rapBin==2) MPhist = MPcanvasTilde_rap2->DrawFrame(PlotpTMin-DeltaXminOVERALL,yMinMP,PlotpTMax,yMaxMP);
              ...
              if(iStateMP==2) MPhist->GetXaxis()->SetNdivisions(504,true);
              if(iStateMP==2 && rapBin==2) MPhist->GetXaxis()->SetNdivisions(508,true);
              MPhist->GetYaxis()->SetNdivisions(505,true);
              ...
              TLegend* MPtildeLegend;
              //MPtildeLegend=new TLegend(0.7,0.75,0.9,0.95);
              //MPtildeLegend=new TLegend(0.8,0.65,0.95,0.85);
              MPtildeLegend=new TLegend(0.8,0.75,0.95,0.95);
              MPtildeLegend->SetFillColor(0);
              //MPtildeLegend->SetTextFont(72);
              MPtildeLegend->SetTextSize(0.05);
              MPtildeLegend->SetBorderSize(0);
              ...
              TGaxis *axisMPY1 = new TGaxis(PlotpTMin-DeltaXminOVERALL,yMinMP,PlotpTMin-DeltaXminOVERALL,yMaxMP,
                                            yMinMP,yMaxMP,AxisDivisions,"-US");
              axisMPY1->SetNdivisions(505);
              axisMPY1->SetTickSize(ticksize);
              axisMPY1->SetTickSize(ticksize/(1-lowestBottomMargin));
              axisMPY1->Draw("same");

              TGaxis *axisMPY2 = new TGaxis(PlotpTMax,yMinMP,PlotpTMax,yMaxMP,yMinMP,yMaxMP,AxisDivisions,"+US");
              axisMPY2->SetNdivisions(505);
              axisMPY2->SetTickSize(ticksize);
              axisMPY2->SetTickSize(ticksize/(1-lowestBottomMargin));
              if(iStateMP==1) axisMPY2->SetTickSize(ticksize/(1-lowestBottomMargin)*0.7);
              axisMPY2->Draw("same");

              TGaxis *axisM3S1 = new TGaxis(PlotpTMin-DeltaXminOVERALL,yMinMP,PlotpTMax,yMinMP,
                                            PlotpTMin-DeltaXminOVERALL+deltaTrickAxisMin,PlotpTMax+deltaTrickAxisMax,AxisDivisions,"+S");
              if(iStateMP==2) axisM3S1->SetNdivisions(504);
              if(iStateMP==2 && rapBin==2) axisM3S1->SetNdivisions(508);
              axisM3S1->SetTickSize(ticksize*2);
              axisM3S1->SetLabelSize(LabelSize); //*0.62
              //if(rapBin==1) axisM3S1->SetLabelSize(LabelSize*(1-Left_margin)); //*0.72
              axisM3S1->SetLabelOffset(labelOffsetX);//*0.7
              ...
              TGaxis *axisM3S2 = new TGaxis(PlotpTMin-DeltaXminOVERALL,yMaxMP,PlotpTMax,yMaxMP,
                                            PlotpTMin-DeltaXminOVERALL+deltaTrickAxisMin,PlotpTMax+deltaTrickAxisMax,AxisDivisions,"-US");
              if(iStateMP==2) axisM3S2->SetNdivisions(504);
              if(iStateMP==2 && rapBin==2) axisM3S2->SetNdivisions(508);
              axisM3S2->SetTickSize(ticksize*2);
              axisM3S2->SetTickSize(ticksize/(1-lowestBottomMargin));
              axisM3S2->Draw("same");

              whereTexteInPlotX=4; //2 //XtitlePosition;
              whereTexteInPlotY=(yMaxMP+yMinMP)/2.-(yMaxMP-yMinMP)*XtitlePositionYshift;
              ...
              MPYtitletext->SetTextSize(YaxistitleLatexSize*(1-lowestBottomMargin));
              MPYtitletext->SetTextColor(kBlack);
              MPYtitletext->PaintLatex(whereTexteInPlotX,whereTexteInPlotY, YtitleAngle, YaxistitleLatexSize*1.2, axislabel);
              MPYtitletext->Draw( "same" );

              char texTexMP[200];
              if(iStateMP==1&&rapBin==1) sprintf(texTexMP,"J/#psi   |#it{y}| < 0.6", nState-5);
              if(iStateMP==1&&rapBin==2) sprintf(texTexMP,"J/#psi    0.6 < |#it{y}| < 1.2", nState-5);
              if(iStateMP==2&&rapBin==1) sprintf(texTexMP,"#psi(2S)    |#it{y}| < 0.6", nState-5);
              if(iStateMP==2&&rapBin==2) sprintf(texTexMP,"#psi(2S)    0.6 < |#it{y}| < 1.2", nState-5);

              xRapTextTilde = PlotpTMax * 0.59 ; //0.66
              if(iStateMP==1) xRapTextTilde = PlotpTMax * 0.71;
              if(rapBin==2) {
                xRapTextTilde = PlotpTMax * 0.48; //0.55
              ...
              if(iStateMP==2){
                char axistitleMPX[200];
                sprintf(axistitleMPX, "#it{p}_{T} [GeV]");
                TLatex *MPXtitletext = new TLatex(5., -1.37, axistitleMPX );
                MPXtitletext->SetTextSize(YaxistitleLatexSize*1.2471);
                MPXtitletext->SetTextColor(kBlack);
                MPXtitletext->Draw( "same" );
              }
              if(iStateMP==1){
                ...
                TLatex *MPXtitletext = new TLatex(65, -1.37, axistitleMPX );
                MPXtitletext->SetTextSize(YaxistitleLatexSize);
                MPXtitletext->SetTextColor(kBlack);
                MPXtitletext->Draw( "same" );
              }
              ...
              //	if(nState==5) CentralsText2MP = new TLatex(PlotpTMax-13.,MPlatexYmax-1.5*MPlatexDeltaYmax,text); //May2
              //	CentralsText2MP->SetTextSize(CentralsFontSizeMP*0.75);
              //	if(DrawPreliminary) CentralsText2MP->Draw( "same" );
              ...
              if(iStateMP==1){
                sprintf(text,"CMS    pp  #sqrt{s} = 7 TeV   L = 4.9 fb^{-1}");
                if(DrawPreliminary) sprintf(text,"CMS Preliminary   pp  #sqrt{s} = 7 TeV   L = 4.9 fb^{-1}");
                TLatex *CentralsText1MP = new TLatex(MPlatexX+3,MPlatexYmax,text);
                CentralsText1MP->SetTextSize(CentralsFontSizeMP*0.75);
                CentralsText1MP->Draw( "same" );
              }
              ...
              //if(iStateMP==2){
              //	sprintf(text,"pp  #sqrt{s} = 7 TeV   L = 4.9 fb^{-1}");
              //	TLatex *CentralsText2MP = new TLatex(PlotpTMax-37,MPlatexYmax,text);
              //	CentralsText2MP->SetTextSize(CentralsFontSizeMP*0.75);
              //	CentralsText2MP->Draw( "same" );
              //}
  ...
  if(SaveTables){
    ...
    if(nState>3)
      sprintf(SuppFileName,"%s/SupplementalMaterial_chic%d.txt",SuppDir,nState-5);
