...
void polFit(int n_sampledPoints=1,
  ...
  if (useAmapApproach) {
    ...
    else if(nDenominatorAmap!=105 && nDenominatorAmap!=106 && nDenominatorAmap!=1)
  ...
  // adjust bins and event numbers for accpetance to broader pt bins in case of chi kinematics
  if(onia::KinParticleChi == true){
    double factor = (pTdilepton_max - pTdilepton_min)/(onia::pTRange[rapBin][pTbin] - onia::pTRange[rapBin][pTbin-1]);
    if(factor < 1) factor = 1;
    std::cout << "correction for chi kinematics = " << factor << std::endl;
    nbinpT = (int)nbinpT*factor;
    n_MCevents_ACC = (int)n_MCevents_ACC*factor;
    n_MCevents_NORM = (int)n_MCevents_NORM*factor;
    std::cout << "nbinPt = " << nbinpT << ", n_MCevents_ACC = " << n_MCevents_ACC << ", n_MCevents_NORM = " << n_MCevents_NORM << std::endl;
  }
  ...
  cout << "end of MC acceptance calculation" << endl;
  cout << endl;
  ...
  for ( int i_event = 1; i_event <= n_events; i_event++ ) {
    ...
    if ( epsilon > min_dileptonEff && isEventAccepted ) {
      // fill histograms
      //background_costhphiPX->Fill( costh_PX, phi_PX, 1. ); // test ilse
      //background_pTrapMass->Fill( pT, TMath::Abs( rap ), mass, 1. ); // test ilse
  ...
  total_costhphiPX->Scale( 1. / total_costhphiPX->Integral() );
  total_costhPX->Scale( n_events / total_costhPX->Integral() );
  total_phiPX->Scale( n_events / total_phiPX->Integral() );

  total_pTrapMass->Scale( 1. / total_pTrapMass->Integral() );
  ...
  // Ilse 2Feb2016: f_background removed to avoid double counting
  background_pTrapMass->Scale( 1.  / background_pTrapMass->Integral() );
  ...
  // Ilse 2Feb2016: f_background removed to avoid double counting
  background_costhphiPX->Scale( 1. / background_costhphiPX->Integral() );
  ...
  for(int iX=0;iX<background_costhphiPX->GetNbinsX()+1;iX++){
    for(int iY=0;iY<background_costhphiPX->GetNbinsY()+1;iY++){
      double BG_content=background_costhphiPX->GetBinContent(iX,iY)*f_background; //Ilse 2Feb2016
      ...
      if(BG_content>Total_content){
        ...
        cout<<"Adapted background model histogram content for cosTheta-phi (bkg = "
            << BG_content << ", tot = " << Total_content << ") at bin X="<<iX<<", Y="<<iY<<endl;
      }
    }
  }
  background_costhphiPX->Scale( 1. / background_costhphiPX->Integral() );
  ...
  for ( int i_event = 1; i_event <= n_events; i_event++ ) {
    ...
    if(StatVarTotBGmodel){
      ...
      // likelihood_BG = likelihood_BG * rnd_costhphiPX / ( nPoisson_costhphiPX );
    ...
    if (    likelihood_BG > likelihood_SpB //removed * f_background
            || likelihood_BG > likelihood_SpB * gRandom->Uniform( 1. )  ) { // Ilse 2Feb2016: gRandom->Uniform( f_background )
  ...
  for(int i_MCevent = 1; i_MCevent <= n_MCevents_NORM; i_MCevent++){
    ...
    pTrapMass_gen->GetRandom3( pT, rap, mass ); // get random events according to cell content

    // pL:
    double rap_sign = gRandom->Uniform(-1., 1.);
    rap_sign /= TMath::Abs(rap_sign);
    rap *= rap_sign;
    double mT = sqrt( mass*mass + pT*pT );
    double pL1 = 0.5 *mT * exp(rap);
    double pL2 = - 0.5 *mT * exp(-rap);
