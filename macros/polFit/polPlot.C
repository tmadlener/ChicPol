...
const int nbin_cth = 40; //40.;//80
const int nbin_ph  = 36; //36.;//72
...
double legend1D_xmin = 0.2,
			 legend1D_ymin = 0.8,
			 legend1D_xmax = 0.75,
			 legend1D_ymax = 0.95;

double latex1D1_left = legend1D_xmin,
			 latex1D1_top  = legend1D_ymin-0.5,
			 latex1D2_left = legend1D_xmax+0.1,
			 latex1D2_top  = legend1D_ymax-0.5;

double yaxisTitleSize1D = 0.05;
...
void polPlot(Char_t *dirstruct = "OutputDirectory_Default",
	...
	if(PlotForPaper) TGaxis::SetMaxDigits(3);
	...
	if(PlotForPaper){
		...
		latex1D1_left = legend1D_xmin;
		latex1D1_top  = legend1D_ymax+0.04;
		latex1D2_left = legend1D_xmax+0.04;
		latex1D2_top  = legend1D_ymax-0.05;

		yaxisTitleSize1D = 0.04;
	...
	if(PlotForPaper) TreeBinIDTextAdd=true;

	TLatex *CentralsText_2D;
	...
	if(TomChange){
		...
		if(!CMSprelim) sprintf(text,"CMS   pp  #sqrt{s} = 7 TeV    L = 4.9 fb^{-1}");
		CentralsText_2D = new TLatex(xCentrals2D,lph_min+(lph_max-lph_min)*0.08,text);
		CentralsText_2D->SetTextSize(CentralsFontSize);
	...
	if(PlotForPaper) CentralsText_2D->Draw( "same" );
	else{
	...
	if(nState==4) sprintf(text,"J/#psi");
	if(nState==5) sprintf(text,"#psi(2S)");
	TreeBinIDText1_2D = new TLatex(xCentrals2D,-0.45,text); //-0.56, -0.73
	...
	TreeBinIDText2_2D = new TLatex(xCentrals2D,-0.56,text); //-0.67, -0.84
	TreeBinIDText2_2D->SetTextSize(CentralsFontSize);
	sprintf(text,"%1.0f < p_{T} < %1.0f GeV",ptlow,pthigh);
	TreeBinIDText3_2D = new TLatex(xCentrals2D,-0.67,text); //-0.78, -0.95
	...
	if(TomChange){
		...
		xCentrals2D=lph_min+(lph_max-lph_min)*0.1;
		sprintf(text,"CMS Preliminary   pp  #sqrt{s} = 7 TeV   L = 4.9 fb^{-1}");
		if(!CMSprelim) sprintf(text,"CMS   pp  #sqrt{s} = 7 TeV    L = 4.9 fb^{-1}");
		CentralsText_2D = new TLatex(xCentrals2D,lph_min+(lph_max-lph_min)*0.08,text);
		CentralsText_2D->SetTextSize(CentralsFontSize);
  ...
  if(PlotForPaper) CentralsText_2D->Draw( "same" );
	...
	if(nState==4) sprintf(text,"J/#psi");
	if(nState==5) sprintf(text,"#psi(2S)");
	TreeBinIDText1_2D = new TLatex(xCentrals2D,-0.45,text); //-0.8
	TreeBinIDText1_2D->SetTextSize(CentralsFontSize);
	if(raplow<0.3) sprintf(text,"|y| < 0.6");
	if(raplow>0.3 && raplow<0.9) sprintf(text,"0.6 < |y| < 1.2");
	if(raplow>0.9) sprintf(text,"1.2 < |y| < 1.5");
	TreeBinIDText2_2D = new TLatex(xCentrals2D,-0.56,text); //-0.91
	TreeBinIDText2_2D->SetTextSize(CentralsFontSize);
	sprintf(text,"%1.0f < p_{T} < %1.0f GeV",ptlow,pthigh);
	TreeBinIDText3_2D = new TLatex(xCentrals2D,-0.67,text); //-1.02
	TreeBinIDText3_2D->SetTextSize(CentralsFontSize);
	...
	if(PlotForPaper) PDF_cth_CS->GetYaxis()->SetTitle(Form("Events / %.2f", 2. / nbin_cth ));
	PDF_cth_CS->GetYaxis()->SetLabelOffset(0.032);
	PDF_cth_CS->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	plotLegend2=new TLegend(legend1D_xmin,legend1D_ymin,legend1D_xmax,legend1D_ymax);
	...
	if(PlotForPaper){
		...
		double left=latex1D1_left, top=latex1D1_top, textSize=0.035;
		...
		latex->SetTextFont(42);
		left = latex1D2_left; top = latex1D2_top;
		if(nState==4) sprintf(text,"J/#psi");
		if(nState==5) sprintf(text,"#psi(2S)");
		latex->DrawLatex(left, top, text);
		top-=step;
	...
	if(PlotForPaper) PDF_ph_CS->GetYaxis()->SetTitle(Form("Events / %.0f degree", 360. / nbin_ph ));
	PDF_ph_CS->GetYaxis()->SetLabelOffset(0.032);
	PDF_ph_CS->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_phth_CS->GetYaxis()->SetTitle(Form("Events / %.0f degree", 360. / nbin_ph ));
	PDF_phth_CS->GetYaxis()->SetLabelOffset(0.032);
	PDF_phth_CS->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_cth_HX->GetYaxis()->SetTitle(Form("Events / %.2f", 2. / nbin_cth ));
	PDF_cth_HX->GetYaxis()->SetLabelOffset(0.032);
	PDF_cth_HX->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_ph_HX->GetYaxis()->SetTitle(Form("Events / %.0f degree", 360. / nbin_ph ));
	PDF_ph_HX->GetYaxis()->SetLabelOffset(0.032);
	PDF_ph_HX->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_phth_HX->GetYaxis()->SetTitle(Form("Events / %.0f degree", 360. / nbin_ph ));
	PDF_phth_HX->GetYaxis()->SetLabelOffset(0.032);
	PDF_phth_HX->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_cth_PX->GetYaxis()->SetTitle(Form("Events / %.2f", 2. / nbin_cth ));
	PDF_cth_PX->GetYaxis()->SetLabelOffset(0.032);
	PDF_cth_PX->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_ph_PX->GetYaxis()->SetTitle(Form("Events / %.0f degree", 360. / nbin_ph ));
	PDF_ph_PX->GetYaxis()->SetLabelOffset(0.032);
	PDF_ph_PX->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_phth_PX->GetYaxis()->SetTitle(Form("Events / %.0f degree", 360. / nbin_ph ));
	PDF_phth_PX->GetYaxis()->SetLabelOffset(0.032);
	PDF_phth_PX->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
	...
	if(PlotForPaper) PDF_calpha->GetYaxis()->SetTitle(Form("Events / %.2f", 2. / nbin_cth ));
	PDF_calpha->GetYaxis()->SetLabelOffset(0.032);
	PDF_calpha->GetYaxis()->SetTitleSize(yaxisTitleSize1D);
