...
void polGen(double rapdilepton_min = 1,
  ...
  TLorentzVector* chic = new TLorentzVector(0.,0.,0.,0.);
  genData->Branch("chic", "TLorentzVector", &chic);
  ...
  for(int i_event = 1; i_event <= n_events; i_event++){
    ...
    // 4-vector:
    TLorentzVector dilepton, chigen;
    dilepton.SetXYZM( pT * cos(Phi) , pT * sin(Phi), pL, mass );

    chigen.SetXYZM( pT * cos(Phi), pT * sin(Phi), pL, mass );
    *chic = chigen;
