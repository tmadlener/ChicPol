...
void bkgHistos_chi(const std::string infilename, int rapBin, int ptBin, bool folding, bool MC, bool PolLSB, bool PolRSB, bool PolNP, int FracLSB, bool normApproach, bool subtractNP,
  ...
  TLorentzVector *lepP = 0, *lepN = 0;
  TLorentzVector *jpsi = 0, *chic = 0, *chic_rf = 0, *particle = 0;
  ...
  std::cout << "-------------------------------------------------------------\n" <<
    "fraction of left sideband: " << fracLSBpsi << " (Jpsi), " << fracLSB1 << " (chic1), " << fracLSB2 << " (chic2)\n" <<
    "combinatorial background fraction: " << fBGsig1 << " (chic1), " << fBGsig2 << " (chic2)\n" <<
    "nonprompt background fraction: " << fNPB1 << " (chic1), " << fNPB2 << " (chic2)\n" <<
    "total background fraction: " << fTBGsig1 << " (chic1), " << fTBGsig2 << " (chic2)" << std::endl;
  ...
  //std::cout << "sigma = " << jpsiMassSigma << ", ymax = " << onia::rapForPTRange[onia::kNbRapForPTBins] << std::endl;
  ...
  std::cout << "-------------------------------------------------------------\n" <<
    ...
    "prompt region: " << PRmin << " < ctau < " << PRmax << "\n"
    "nonprompt region: " << NPmin << " < ctau < " << NPmax << "\n"
    "-------------------------------------------------------------\n" << std::endl;
  ...
  // in case of chi kinematics find pT and rapidity borders in dimuon kinematics
  int n = intree->GetEntries();
  TH1D *htemp_pt = new TH1D("","", 1000, 0, 100);
  TH1D *htemp_y = new TH1D("", "", 240, 0, 2.4);
  ...
  if(onia::KinParticleChi == true){
    ...
    for(int i = 0; i < n; i++){
      ...
      if(particle->Pt() >= onia::pTRange[rapBin][ptBin-1] &&
         ...
         TMath::Abs(particle->Rapidity()) < onia::rapForPTRange[rapBin] &&
         TMath::Abs(jpsi->M() - CBmass_p0) < onia::nSigMass * rapSigma(p0, p1, p2, jpsi->Rapidity()) ){

        htemp_pt->Fill(jpsi->Pt());
        htemp_y->Fill(TMath::Abs(jpsi->Rapidity()));

      } // if particle
    } //for

    double tempPt1 = htemp_pt->GetBinLowEdge(htemp_pt->FindFirstBinAbove());
    double tempPt2 = htemp_pt->GetBinLowEdge(htemp_pt->FindLastBinAbove()+1);
    double tempY1 = htemp_y->GetBinLowEdge(htemp_y->FindFirstBinAbove());
    double tempY2 = htemp_y->GetBinLowEdge(htemp_y->FindLastBinAbove()+1);
    minPt = (floor(tempPt1*2)/2);
    maxPt = (ceil(tempPt2*2)/2);
    minRap = (floorf(tempY1*10)/10);
    maxRap = (ceilf(tempY2*10)/10);
    std::cout << "first bins = " << tempPt1 << ", " << tempY1 << "; last bins = " << tempPt2 << ", " << tempY2 << std::endl;
    if(maxRap < onia::rapForPTRange[rapBin]) maxRap = onia::rapForPTRange[rapBin];

  } // if KinParticleChi

  std::cout << "--------------------------------------------\n"
            << "pT binning of jpsi: " << minPt << " - " << maxPt << "\n"
            << "rapidity binning of jpsi: " << minRap << " - " << maxRap << std::endl;

  //------------------------------------------------------------------------------------------------
  // build the 3D (pT, |y|, M) histos for the L and R mass sideband
  // prompt mass sidebands using background function
  TH3D* hBG1_pTRapMass_L = new TH3D("hBG_pTRapMass_L", ";p_{T} [GeV/c]; |y|; M [GeV]",
                                    7, minPt, maxPt,
                                    2, minRap, maxRap, //onia::kNbRapForPTBins
  ...
  // non prompt mass sidebands using background function
  ...
  for(int iFrame = 0; iFrame < onia::kNbFrames; iFrame++){
    ...
    if(iFrame==0) std::cout << "cosTheta-phi" << std::endl;
    ...
    if(iFrame == 0){
      std::cout << "background histogram output:\n"
                << "comb.: " << hBG1_cosThetaPhi[iFrame]->GetBinContent(3,12) << "\n"
                << "tot.: " << hTBG1_cosThetaPhi[iFrame]->GetBinContent(3,12)
                << std::endl;
    }
    ...
    if(iFrame == 0){
      std::cout << "final tot.: " << hTBG1_cosThetaPhi[iFrame]->GetBinContent(3,12)
                << std::endl;
    }
